...
% 	coloredObjectsMask = cast(coloredObjectsMask, class(rgbImage));
	...
	% Measure the Mean H HSV and area of all the detected blobs.
	[meanHSV, areas, numberOfBlobs] = MeasureBlobs(coloredObjectsMask, hImage, sImage, vImage,coloredObjectsMask);
	if numberOfBlobs > 0
		fprintf(1, '\n----------------------------------------------\n');
...
%----------------------------------------------------------------------------
function [meanHSV, areas, numberOfBlobs] = MeasureBlobs(maskImage, hImage, sImage, vImage,coloredObjectsMask)
try
	[labeledImage, numberOfBlobs] = bwlabel(maskImage, 4);     % Label each blob so we can make measurements of it
    
    cc = bwconncomp(coloredObjectsMask, 4);
        cc.NumObjects;
        graindata = regionprops(cc, 'basic');
     assignin('base', 'graindata', graindata);
   
     whiteFeaturesCentroids = regionprops(cc, 'centroid');
     assignin('base', 'whiteFeaturesCentroids', whiteFeaturesCentroids);
      
     for i=1:numel(whiteFeaturesCentroids)
        whiteFeaturesCentroids(i).Centroid;
         
     end
     
     whiteFeaturesCentroidsMat = zeros(numel(whiteFeaturesCentroids),2);
     
     
     for elem = 1:numel(whiteFeaturesCentroids)
         whiteFeaturesCentroidsMat(elem,:)= whiteFeaturesCentroids(elem).Centroid;
     end
     
     assignin('base', 'whiteFeaturesCentroidsMat', whiteFeaturesCentroidsMat);
